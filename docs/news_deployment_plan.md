...
| `RACE_RESULTS_UPDATE_INTERVAL_MINUTES` | `360` | 레이스 결과(F1DB) 갱신 주기 — 동작 방식은 순위와 동일 |
...
| `RACE_RESULTS_JSON_REMOTE_URL` | `https://<railway-domain>/race-results.json` | 원격 레이스 결과 소스(`/api/race-results`). 미설정이면 앱이 "준비 중" 카드 유지 |
...
curl "https://<railway-domain>/race-results.json" # 종료된 라운드 수만큼 races 기대
...
curl "https://<vercel-domain>/api/race-results?season=2026&raceId=great-britain-2026"
```
