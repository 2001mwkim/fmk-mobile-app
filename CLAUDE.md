...
- `test/` — 주제별로 파일을 나눈다(현재 23개). 화면: app_navigation / app_ui / tablet_layout / home_hero / home_cards / home_recent_result / live_center_screen / standings_detail / news_screen / my_picks. 로직: live_session_model / live_session_controller / live_session_label / live_widgets / standing_insights. 서비스: news_repository / race_results_repository / standings_repository / notification_settings_controller / widget_theme_controller / fmk_home_widget_bridge / live_session_service. 규약: app_version(앱 정보 화면 버전 표기 `lib/app_version.dart` 가 pubspec `version:` 과 어긋나면 실패 — 릴리스 때 두 곳을 같이 올릴 것)

## 컨벤션

- **색상**: 팔레트는 전부 `lib/theme/app_colors.dart`의 `AppColors`. 새 hex 리터럴을 화면에 직접 넣지 말 것(웹 Tailwind 값만 옮겨온다는 원칙). **노란색 금지**(P1 강조도 레드 사용). 유일한 승인 예외: 타이어 미디엄 컴파운드의 피렐리 마킹색 `AppColors.tyreMedium`
