...
포뮬러 매거진 코리아의 F1 팬용 Flutter 앱(Android 우선). 2026 시즌 일정·순위·직관 가이드·라이브 타이밍을 제공한다. 기존 Next.js 웹앱의 UI를 이식한 프로젝트라서, 화면 코드 주석에 웹 컴포넌트 출처(`components/...tsx`)와 색상 원본값을 적는 관례가 있다.
